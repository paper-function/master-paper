classdef cooperativeGuidanceTest < matlab.unittest.TestCase
    ...
    methods (Test)
        ...
        function testTerminalRangeUsesPurePng(testCase)
            [position, velocity, target, adjacency, params] = sampleScenario();
            for vehicleIndex = 1:3
                position(:, vehicleIndex) = target - [2000; 0; 0];
                velocity(:, vehicleIndex) = [2700; 0; 0];
            end

            [~, info] = cooperativeFuzzyGuidance3D(position, velocity, target, ...
                [0; 0; 0], adjacency, params);

            testCase.verifyEqual(info.cooperativeWeight, 0, 'AbsTol', 1e-12);
            testCase.verifyEqual(info.localCooperativeWeight, zeros(1, 3), ...
                'AbsTol', 1e-12);
        end

        function testFuzzyBlendUsesOneGroupWeight(testCase)
            [position, velocity, target, adjacency, params] = sampleScenario();

            [~, info] = cooperativeFuzzyGuidance3D(position, velocity, target, ...
                [0; 0; 0], adjacency, params);

            testCase.verifyEqual(info.localCooperativeWeight, ...
                repmat(info.cooperativeWeight, 1, 3), 'AbsTol', 1e-12);
        end
