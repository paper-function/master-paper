%% Priority fix step 1: tune bank-angle TD speed r_miu
% This script keeps the original controller code unchanged and only varies
% the workspace parameter r_miu used by FlowAngle_Controller/TD_miu.
...
outDir = fullfile(rootDir, 'terminal_adrc_results', 'priority_fixes', 'step1_miu_td');
...
rMiuValues = [0.03 0.10 0.24];
cases = defineCases();
rows = [];
results = struct([]);

for i = 1:numel(rMiuValues)
    for j = 1:numel(cases)
        fprintf('r_miu=%0.3f case=%s\n', rMiuValues(i), cases(j).name);
        r = runCase(harness, cases(j), xTemplate, rMiuValues(i));
        results(end+1).r = r; %#ok<SAGROW>
        if r.ok
            m = r.metrics;
            rows = [rows; table(rMiuValues(i), string(cases(j).name), true, m.stable, ...
                m.downpress, m.dH, m.avgHdot, m.minThetaDeg, m.maxAlphaErrDeg, ...
                m.maxMiuErrDeg, m.tailMiuRmsDeg, m.maxDeltaDeg, m.satFrac, ...
                'VariableNames', {'r_miu','case','ok','stable','downpress','dH','avgHdot', ...
                'minThetaDeg','maxAlphaErrDeg','maxMiuErrDeg','tailMiuRmsDeg','maxDeltaDeg','satFrac'})]; %#ok<AGROW>
        else
            rows = [rows; table(rMiuValues(i), string(cases(j).name), false, false, ...
...
writetable(rows, fullfile(outDir, 'step1_miu_td_metrics.csv'));
save(fullfile(outDir, 'step1_miu_td_results.mat'), 'rows', 'results', 'cases', 'rMiuValues');
plotSummary(rows, figDir);
writeReport(fullfile(outDir, 'step1_miu_td_report.md'), rows);
fprintf('Done: %s\n', outDir);
...
function r = runCase(model, c, xTemplate, rMiu)
r = struct('ok', false, 'caseDef', c, 'r_miu', rMiu, 'error', '', 'metrics', [], 'data', []);
xInit = xTemplate;
...
for i = 1:numel(cases)
    ...
    nexttile(1); hold on; plot(sub.r_miu, sub.maxMiuErrDeg, '-o', 'DisplayName', cases(i));
    nexttile(2); hold on; plot(sub.r_miu, sub.tailMiuRmsDeg, '-o', 'DisplayName', cases(i));
    nexttile(3); hold on; plot(sub.r_miu, sub.maxDeltaDeg, '-o', 'DisplayName', cases(i));
    nexttile(4); hold on; plot(sub.r_miu, sub.avgHdot, '-o', 'DisplayName', cases(i));
end
nexttile(1); grid on; ylabel('max miu error deg'); xlabel('r_miu'); legend('Location','best');
nexttile(2); grid on; ylabel('tail miu RMS deg'); xlabel('r_miu');
nexttile(3); grid on; ylabel('max surface deg'); xlabel('r_miu');
nexttile(4); grid on; ylabel('avg Hdot m/s'); xlabel('r_miu');
exportgraphics(fig, fullfile(figDir, 'step1_miu_td_summary.png'));
close(fig);
end

function writeReport(file, rows)
fid = fopen(file, 'w'); c = onCleanup(@() fclose(fid));
fprintf(fid, '# Step 1: bank TD r_miu tuning\n\n');
fprintf(fid, '| r_miu | case | ok | stable | downpress | max miu err | tail miu RMS | max surface | avg Hdot |\n');
fprintf(fid, '|---:|---|---:|---:|---:|---:|---:|---:|---:|\n');
for k = 1:height(rows)
    fprintf(fid, '| %.3f | %s | %d | %d | %d | %.2f | %.2f | %.2f | %.1f |\n', ...
        rows.r_miu(k), rows.case(k), rows.ok(k), rows.stable(k), rows.downpress(k), ...
