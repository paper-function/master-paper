...
sizes.NumContStates  = 14; %经度，纬度，x，y，H，V，航迹方位角，航迹��斜角，迎角，侧滑角，��侧角，滚转角��度，俯仰角速度，偏航角速度
...
[Ma,Q] = M_Q(H0,V0);
x0  = [0,0,0,0,H0,V0,psi0,theta0,0*pi/180,0*pi/180,0*pi/180,0*pi/180,0*pi/180,0*pi/180]; %���ȣ�γ�ȣ�x��y��H��V��������λ�ǣ�������б�ǣ�ӭ�ǣ��໬�ǣ����ǣ���ת���ٶȣ��������ٶȣ�ƫ�����ٶ�
...
global D Y1;
Parameter;
%状��向量，各分量依次为 经度，纬度，X，Y，H，V，航迹偏角，航迹倾角，迎角，侧滑角，倾侧角，滚转角��度，俯仰角速度，偏航角速度
...
b_alpha = Dot_flowangle(1); b_beta = Dot_flowangle(2); b_miu = Dot_flowangle(3); %模型辅助�?phi_alpha = phi_s(1); phi_beta = phi_s(2); phi_miu = phi_s(3); %模型不确定��与干扰
...
b_p = Dot_anglerate(1); b_q = Dot_anglerate(2); b_r = Dot_anglerate(3); %模型辅助�?phi_p = phi_f(1); phi_q = phi_f(2); phi_r = phi_f(3); %模型不确定��和外扰
