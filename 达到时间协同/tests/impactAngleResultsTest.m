classdef impactAngleResultsTest < matlab.unittest.TestCase
    ...
    methods (Test)
        ...
        function testInitialAndTerminalAnglesDiffer(testCase)
            projectFolder = fileparts(fileparts(mfilename("fullpath")));
            loaded = load(fullfile(projectFolder, "impact_angle_results", ...
                "impact_angle_summary.mat"), "summary");
            thetaChange = abs([loaded.summary.terminalThetaDeg] - ...
                [loaded.summary.thetaInitialDeg]);
            psiChange = abs([loaded.summary.terminalPsiDeg] - ...
                [loaded.summary.psiInitialDeg]);
            testCase.verifyGreaterThanOrEqual(thetaChange, 9.0);
            testCase.verifyLessThanOrEqual(thetaChange, 11.0);
            testCase.verifyGreaterThanOrEqual(psiChange, 4.0);
            testCase.verifyLessThanOrEqual(psiChange, 6.0);
        end
